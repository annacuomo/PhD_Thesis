...
For simplicity, I use the same notation for multivariate normal (e.g. $ \mathcal{N}(\boldsymbol{\mu}, \boldsymbol{\Sigma})$), noting that the specified parameters ($\boldsymbol{\mu}$ and $\boldsymbol{\Sigma}$) are a $N \times 1$ mean vector, and a $N \times N$ covariance matrix, respectively.
...
Additionally, these tests are not equipped to account for confounding factors (these are described in more detail in section \ref{sec:linear_mixed_models}).
...
The Neyman-Pearson lemma \cite{neyman1933ix} proves that the power  of the \gls{lrt}, defined as the probability of rejecting the null hypothesis when it is false: $P = P($reject $H_0 | H_1)$ (Box\ref{box:confusion_mat}) is the same or higher than the power of the score test:
...
As an example, in the method described in \cite{ongen2016fast}, the minimal p value for each permutation across all SNPs, and use those to build a beta distribution of background p values, against which to compare the real p values.
...
Traditionally `confounders' are those variables that are correlated with both the genotype vector and the phenotype, i.e. hidden common causes of both $\mathbf{g}$ an  $\mathbf{y}$. 
