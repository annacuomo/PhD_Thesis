...
where $\hat{\theta}$ is the MLE for $\theta$ (value of $\theta$ that maximisise the likelihood) and $\theta_0$ is the value $theta$ assumes under $H_0$.
...
One advantag of the \gls{lrt} is that it is robust to re-parametrisation of the parameters, whereas the Score test is not (for completeness, neither is the Wald test). 
...
In terms of removing effects of population stratification, \gls{lmm}s have been shown to perform exactly as well as  including all M PCs (or regressing them out), which is of course not feasible in practice.
...
To do so, we perform eigen decomposition of $\mathbf{K}$ \eqref{eq:LMM_MVN}, such that: $\mathbf{K} = \mathbf{Q}\mathbf{S}\mathbf{Q}^T$.\\
...
For every \gls{snp}, we find the \gls{mle}s for $\beta$ and $\sigma^2_g$ using the closed form from ().
...
Because we are performing \textit{cis} \gls{eqtl} mapping, we define a \textit{cis} window around the gene (typically ranging from 100kb to 1Mb) and a minimum \gls{maf} (e.g. 0.05) and test all \gls{snp}s within the window with \gls{maf}>0.05 within our population.  
We test one \gls{snp} at the time, and only the alternative model is changing.
...
As described in chapter 1 (section X), a significant statistical \gls{gxe} interaction is defined when there is a significant difference in the genetic effect between two groups of individuals with different environmental exposures (Fig. \ref{fig:gxe}).
...
where $\odot$ denotes element-wise multiplication (Hadamard product), such that $\mathbf{g} \odot \boldsymbol{\beta_{GxE}} = diag(\mathbf{g})\boldsymbol{\beta_{GxE}}$ where $diag(\mathbf{g})$ is an NxN diagonal matrix, i.e. all off-iagonal elements are 0, diagonal elements correspond to elements of the Nx1 vector $\mathbf{g}$, whereas $\boldsymbol{\beta_{GxE}}$ is a Nx1 vector such that
